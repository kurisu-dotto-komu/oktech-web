...
test.describe("Events Page Prefetching", () => {
  ...
  test("should prefetch filtered events when using filters", async ({ page }) => {
    ...
    const initialEventCount = await page.$$eval('[data-testid*="event-card"]', els => els.length);
    ...
    if (filterButton) {
      ...
      const filteredEventCount = await page.$$eval('[data-testid*="event-card"]', els => els.length);
